...
Item { // Player instance
    ...
    Process { // Launch music player
        id: ytPlayerProc
        command: ["bash", "-c", "yt-player &"]
    }
    ...
    Rectangle { // Background
        ...
        RowLayout {
            ...
            ColumnLayout { // Info & controls
                ...
                RowLayout { // Shuffle, loop, volume, open player
                    Layout.fillWidth: true
                    spacing: 4

                    RippleButton { // Shuffle toggle
                        implicitWidth: 24
                        implicitHeight: 24
                        ...
                        contentItem: MaterialSymbol {
                            ...
                            horizontalAlignment: Text.AlignHCenter
                            text: "shuffle"
                            color: (root.player?.shuffle ?? false) ? blendedColors.colPrimary : blendedColors.colOnSecondaryContainer
                            Behavior on color {
                                animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(this)
                            }
                        }
                    ...
                    RippleButton { // Loop/repeat cycle
                        implicitWidth: 24
                        implicitHeight: 24
                        ...
                        contentItem: MaterialSymbol {
                            ...
                            fill: 1
                            horizontalAlignment: Text.AlignHCenter
                            text: (root.player?.loopState ?? MprisLoopState.None) === MprisLoopState.Track ? "repeat_one" : "repeat"
                            color: (root.player?.loopState ?? MprisLoopState.None) !== MprisLoopState.None ? blendedColors.colPrimary : blendedColors.colOnSecondaryContainer
                            Behavior on color {
                                animation: Appearance.animation.elementMoveFast.colorAnimation.createObject(this)
                            }
                        }
                    ...
                    RippleButton { // Open music player
                        implicitWidth: 24
                        implicitHeight: 24
                        downAction: () => { ytPlayerProc.running = true; }
                        colBackground: ColorUtils.transparentize(blendedColors.colSecondaryContainer, 1)
                        colBackgroundHover: blendedColors.colSecondaryContainerHover
                        colRipple: blendedColors.colSecondaryContainerActive
                        contentItem: MaterialSymbol {
